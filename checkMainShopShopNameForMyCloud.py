...
def with_other(str):
    for ch  in str:
        if is_other(ch):
            return  True
    return  False
def checkShopName(db):
    page_size = 10000
    i = 1.0
    last_row_id = ''
    content = db.MainShop5.find(
        # filter={
        #     "_id":{'$gt':ObjectId('57a0bb69951f59b1cf837370')}
        # }
    ...
    while True:
        ...
        for json in content:
            ...
            try:
                ...
                # 假如处理的方法
                if with_other(shopName):
                    result = db.MainShop3.update_one({'_id': json[u'_id']}, {'$set': {"shopName":shopName}}, True)
                    if result.upserted_id != None:
                        print shopName,"更新成功"
            except Exception, e:
                print e, "Exception"
