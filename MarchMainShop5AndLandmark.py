...
content = db.MainShop5.find(
    filter={
        '_id':{'$gt':ObjectId('57a0498b951f59b1cfa77e26')}
    }
...
while True:
    ...
    for json in content:
        ...
        try:
            coordinates = json[u'coordinates']
            land_arr=[]
            for word in landmark_dic:
                land_coor =  landmark_dic.get(word)
                d = getGDDistance(coordinates,land_coor)
                if d<1.0:
                    land_arr.append(word)
            result = db.MainShop5.update_one({'_id':json[u'_id']},{'$set':{'landArr':land_arr}})
