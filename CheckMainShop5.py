...
def mainshop5_check_len(json, check_len_count):
    ...
    if check_len_count%10000==0:
         print 'city',len(str(json[u'city'])),'brand',len(json[u'brand']),'shopName',len(json[u'shopName']),'address',len(json[u'address']),'description',len(json[u'description']), 'coordinates',(json[u'coordinates']), 'tradingArea',len(json[u'tradingArea']), 'hours',len(json[u'hours'])
...
while True:
    ...
    for json in content:
        ...
        try:
            check_type_count = mainshop5_check_type(json, check_type_count)
            check_len_count = mainshop5_check_len(json, check_len_count)
            check_standard_count = mainshop5_check_standardized(json, check_standard_count)
            if check_len_count%10000==0:
